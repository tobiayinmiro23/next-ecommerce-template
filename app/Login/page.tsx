...
const page = () => {
    ...
    const login = () => {
        ...
        setloading(true)
        fetch('http://localhost:3000/api/Ecommerce/Login', {
    ...
    return (
        ...
        <div className="mt-[12rem] w-[24rem] m-[auto]">
            ...
            <input type="text" className="block mb-[1rem] p-[0.3rem] pl-[0.8rem] outline-none rounded-md border-[1px] border-[#c8c5c5] w-[85%]" placeholder="password" value={password} onChange={(e) => setpassword(e.target.value)} />
            {
                loading ?
                    ...
                    :
                    <button type="submit" className="block bg-[#19afe0] py-[0.4rem] rounded-md text-white font-bold w-[85%]" onClick={login}>login</button>
            }
        </div>
    )
}
